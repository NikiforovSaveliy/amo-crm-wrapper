...
def get_fields_for_entity(entity: EntityType, field_klass: Generic[T]) -> Dict[str, T]:
    type_hints = get_type_hints(entity, include_extras=True)
    type_hints = filter_dict_by_class(type_hints, field_klass)
    result = dict()

    for attr, hint in type_hints.items():
        actual_type, field = get_args(hint)
        result[attr] = field
